/**
 * Sync riders/teams that exist locally but are missing in PROD (TiDB).
 * Also sync race stages (LOCAL -> PROD) for the given race codes so the
 * stage UUIDs match between environments.
 *
 * Usage: node sync_meta_prod.js <race-code> [<race-code> ...]
 */
...
(async () => {
  const raceCodes = process.argv.slice(2);
  const l = await mysql.createConnection(LOCAL);
  const p = await mysql.createConnection(PROD);
  p.on('error', () => {});

  // 1. stages for the given races (same UUIDs)
  for (const code of raceCodes) {
    const [race] = await q(l, 'SELECT id FROM races WHERE race_code=?', [code]);
    if (!race.length) { console.log(code, 'race NOT FOUND'); continue; }
    const raceId = race[0].id;
    const [stages] = await q(l, 'SELECT * FROM stages WHERE race_id=? ORDER BY stage_number', [raceId]);
    for (const t of ['stage_results', 'general_classification', 'points_classification', 'mountains_classification', 'youth_classification', 'team_classification', 'jerseys']) {
      await q(p, `DELETE FROM ${t} WHERE stage_id IN (SELECT id FROM stages WHERE race_id=?)`, [raceId]);
    }
    ...
    for (const s of stages) {
      await q(p, `INSERT INTO stages (id, race_id, stage_number, stage_name, stage_name_zh, stage_type, date,
        ...
        weather_summary, stage_code, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
  ...
  console.log(`riders synced to PROD: ${rAdded}`);

  // 3. teams missing in PROD
