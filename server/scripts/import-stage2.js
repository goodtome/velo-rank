 ...
 * 导入 Stage 2 数据到数据库
 * 数据来源: PCS HTML 解析结果
 * 赛段: Stage 2 - Naples → Naples (2026-05-09, 200km)
 */
...
// 读取解析好的数据
const dataContent = fs.readFileSync('./stage2-results-data.js', 'utf-8');
const match = dataContent.match(/const STAGE_RESULTS_DATA = (\[[\s\S]*?\]);/);
if (!match) {
  console.error('❌ 无法解析 stage2-results-data.js');
  process.exit(1);
}
...
// 领骑衫数据（来自PCS解析）
const JERSEYS_DATA = [
  { jersey_type: 'pink',        rider: 'Magnier Paul',          team: 'Soudal Quick-Step'         },
  { jersey_type: 'BLUE_SPRINT', rider: 'Sevilla Diego Pablo',   team: 'Team Polti VisitMalta'     },
  { jersey_type: 'WHITE_YOUTH', rider: 'Morgado António',       team: 'UAE Team Emirates - XRG'   },
];

// Stage 2 赛段信息
const STAGE_INFO = {
  stage_number: 2,
  stage_name: 'Naples → Naples',
  date: '2026-05-09',
  distance_km: 200,
  stage_type: 'Hills',
  stage_code: 'giro-2026-s2'
};

console.log('🚴 Stage 2 数据导入工具\n');
console.log('='.repeat(60));
console.log(`赛事: Giro d'Italia 2026`);
console.log(`赛段: Stage 2 - ${STAGE_INFO.stage_name} (${STAGE_INFO.date}, ${STAGE_INFO.distance_km}km)`);
console.log(`成绩数据: ${stageResults.length} 条`);
...
async function main() {
  ...
  try {
    ...
    const processedResults = stageResults.map(r => {
      ...
      // 第1次出现用原始 rank，第2次及以后用 rawRank + count - 1（保持递增）
    ...
    for (const result of processedResults) {
      try {
        ...
        if (imported <= 10 || imported % 30 === 0) {
          console.log(`  ✅ ${String(result.rank).padStart(3)}. ${result.rider_name.padEnd(30)} (${result.team_name.substring(0, 25)}) ${result.time_gap}`);
        }
      } catch (err) {
        skipped++;
        if (skipped <= 5) {
          console.error(`  ❌ 失败 [rank=${result.rank}] ${result.rider_name}:`, err.message);
        }
      }
    }
    ...
    console.log('🏆 数据库验证 - Stage 2 前10名：');
