...
# Stage 17 的 ID
STAGE_ID = 'fe6ae4b2-f6fd-4b26-8ac8-1c65b187df52'
...
def get_or_create_team(cursor, team_name):
    ...
    sql = """INSERT INTO teams (id, team_name, team_name_en, team_name_zh, created_at, updated_at)
...
def import_jerseys(cursor, data):
    ...
    for j in jerseys:
        ...
        # 获取或创建车手和车队
        rider_uuid = get_or_create_rider(cursor, rider_name, rider_slug)
        team_uuid = get_or_create_team(cursor, team_name)
...
def import_gc(cursor, data):
    ...
    for r in results:
        ...
        # 插入数据（GC表有 team_id, nationality, total_time, time_gap）
        record_id = str(uuid.uuid4())
        total_time = r.get('time', '')  # PCS 数据中的 'time' 字段对应 total_time
...
def import_youth(cursor, data):
    ...
    for r in results:
        ...
        # 插入数据（Youth表有 time, time_gap, jersey_type, updated_at）
        # 注意：Youth表的 id 是 int 自增，不需要提供
        time_val = r.get('time', '')
        time_gap = r.get('time_gap', '')
        ...
        cursor.execute(sql, (
            STAGE_ID, rider_uuid,
            int(rank), time_val, time_gap, 'WHITE'
        ))
        inserted += 1
...
def import_points(cursor, data):
    ...
    for r in results:
        ...
        # 插入数据（Points表有 points, jersey_type, updated_at）
        # 注意：Points表的 id 是 int 自增，不需要提供
...
def import_kom(cursor, data):
    ...
    for r in results:
        ...
        # 插入数据（KOM表有 points, jersey_type, updated_at）
        # 注意：KOM表的 id 是 int 自增，不需要提供
        points = r.get('points', 0)
