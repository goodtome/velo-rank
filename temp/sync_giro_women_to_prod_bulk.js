  ...
  try {
    ...
    for (const s of localStages) {
      await prodConn.query(
        'INSERT INTO stages (id, race_id, stage_number, stage_name, date, distance_km, stage_code) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE stage_name=VALUES(stage_name), date=VALUES(date), distance_km=VALUES(distance_km)',
    ...
    for (const stageId of stageIds) {
      ...
      const teamKeys = [...new Set(results.map(r => r.team_slug || r.team_name))];

      // Fetch/Create Riders and Teams in Prod
      ...
      for (const res of results) {
        if (!riderMap.has(res.rider_slug)) {
          ...
          if (prodRiders.length > 0) riderMap.set(res.rider_slug, prodRiders[0].id);
          else {
            const id = uuidv4();
            await prodConn.query('INSERT INTO riders (id, rider_name, rider_slug, nationality, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())', [id, res.rider_name, res.rider_slug, res.rd_nat]);
            riderMap.set(res.rider_slug, id);
          }
        }
        const tKey = res.team_slug || res.team_name;
        if (!teamMap.has(tKey)) {
          const [prodTeams] = await prodConn.query('SELECT id FROM teams WHERE team_slug = ? OR team_name = ?', [res.team_slug, res.team_name]);
          if (prodTeams.length > 0) teamMap.set(tKey, prodTeams[0].id);
          else {
            const id = uuidv4();
            await prodConn.query('INSERT INTO teams (id, team_name, team_slug, uci_code, created_at) VALUES (?, ?, ?, ?, NOW())', [id, res.team_name, res.team_slug, res.uci_code]);
            teamMap.set(tKey, id);
          }
        }
      }
      ...
      const resultValues = results.map(r => [uuidv4(), stageId, riderMap.get(r.rider_slug), teamMap.get(r.team_slug || r.team_name), r.rank_pos, r.time_gap, r.nationality, new Date()]);
      if (resultValues.length > 0) {
        await prodConn.query('INSERT INTO stage_results (id, stage_id, rider_id, team_id, rank_pos, time_gap, nationality, created_at) VALUES ?', [resultValues]);
      }
      ...
      const gcValues = gc.map(g => [uuidv4(), stageId, g.rank, riderMap.get(g.rider_slug), teamMap.get(g.team_slug || g.team_name), g.nationality, g.total_time, g.time_gap, new Date()]);
      if (gcValues.length > 0) {
        await prodConn.query('INSERT INTO general_classification (id, stage_id, `rank`, rider_id, team_id, nationality, total_time, time_gap, created_at) VALUES ?', [gcValues]);
      }
